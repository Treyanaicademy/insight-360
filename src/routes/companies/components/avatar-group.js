...
export const AvatarGroup = ({ avatars, size, overlap, maxCount = 3, gap = "8px", containerStyle, avatarStyle, }) => {
    ...
    return (React.createElement("div", { style: {
        ...
        visibleAvatars.map((avatar, index) => {
            const transform = shouldOverlap
                ...
                : undefined;
            return (React.createElement(Tooltip, { title: avatar.name, key: index },
                React.createElement(CustomAvatar, { style: {
                        ...
                        ...avatarStyle,
                    }, name: avatar?.name, src: avatar?.src, size: size })));
        }),
        hasRemainingAvatars && (React.createElement(Tooltip, { destroyTooltipOnHide: true, title: React.createElement(Space, { direction: "vertical" }, remainingAvatars.map((avatar, index) => {
                return (React.createElement(Space, { key: index },
                    React.createElement(CustomAvatar, { name: avatar.name, src: avatar.src, size: "small" }),
                    React.createElement(Text, { style: {
                            color: "#fff",
                        }, key: avatar.name }, avatar.name)));
            })) },
            React.createElement(Text, { className: "tertiary", style: {
                    userSelect: "none",
                    cursor: "pointer",
                    fontSize: "10px",
                    lineHeight: "22px",
