...
// Define an interface for the color object
...
type FormValues = EventCreateInput & {
  ...
  color: string | ColorObject; // Updated to include ColorObject
};
...
export const CalendarCreatePage: React.FC = () => {
  ...
  const handleOnFinish = async (values: FormValues) => {
    ...
    await onFinish({
      ...
      color: typeof color === "object" && color !== null
        ? `#${(color as ColorObject).toHex()}`  // Type assertion used here
        : color,
    });
  };
